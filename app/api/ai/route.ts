...
const EXTRACTION_SYSTEM_PROMPT = `You are a DIY project assistant. Extract project details into this EXACT JSON structure. Do NOT use "dimensions" arrays or "preferences" arrays.
...
export async function POST(request: Request) {
  try {
    ...
    const knownQuestions = !isRefine ? getFilteredKnownQuestions(String(projectType), input) : [];
    const aiQuestions = Array.isArray(clarifyingQuestions) ? clarifyingQuestions : [];
    const mergedQuestions = [...knownQuestions, ...aiQuestions];

    return NextResponse.json({ extracted: { ...extracted, clarifyingQuestions: mergedQuestions }, estimate });
  } catch (error) {
